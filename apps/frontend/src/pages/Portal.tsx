...
export const Portal = () => {
  ...
  return (
    <Card>
      ...
      <CardContent>
        <p className="text-sm text-slate-600">{t('invoice:title')} · Accédez à vos bulletins d’achat et soldes.</p>
        <div className="mt-4 overflow-hidden rounded-xl border border-slate-200">
          <table className="w-full text-sm">
            <thead className="bg-slate-100 text-left text-xs uppercase tracking-wide text-slate-500">
              <tr>
                <th className="px-4 py-3">{t('invoice:number', { defaultValue: 'N°' })}</th>
            ...
            <tbody>
              {invoices.map((invoice) => (
                <tr key={invoice.number} className="border-t border-slate-200">
                  <td className="px-4 py-3 font-semibold text-emerald-700">{invoice.number}</td>
                  <td className="px-4 py-3">{invoice.date}</td>
                  <td className="px-4 py-3 font-medium text-emerald-700">
                    {new Intl.NumberFormat('fr-DZ', { style: 'currency', currency: 'DZD' }).format(invoice.amount)}
                  </td>
                </tr>
              ))}
            </tbody>
