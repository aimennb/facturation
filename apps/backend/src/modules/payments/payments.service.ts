...
export class PaymentsService {
  ...
  async create(payload: CreatePaymentDto, userId?: string) {
    const supplier = await this.suppliersRepository.findOne({ where: { id: payload.supplierId } });
    if (!supplier) {
      throw new NotFoundException('Supplier not found');
    }
    if (payload.invoiceId) {
      const invoice = await this.invoicesRepository.findOne({ where: { id: payload.invoiceId } });
      if (!invoice) {
    ...
    await this.auditService.log('payment.created', 'Payment', saved.id, payload as any, userId);
    return saved;
  }
  ...
  async remove(id: string, userId?: string) {
    ...
    await this.auditService.log('payment.deleted', 'Payment', id, undefined, userId);
    return { success: true };
  }
}
