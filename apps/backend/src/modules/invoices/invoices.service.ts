...
export class InvoicesService {
  ...
  private ensureWeights(item: InvoiceItemDto) {
    if (item.weightBrut < item.weightTare) {
      throw new BadRequestException(
        "Brut weight must be greater than or equal to tare",
      );
    }
  }
  ...
  async create(createInvoiceDto: CreateInvoiceDto, userId?: string) {
    ...
    try {
      ...
      const { seq, number, settings } = await this.generateIdentifiers(
        queryRunner.manager,
        date,
      );
      ...
      const items = createInvoiceDto.items.map((item) =>
        this.mapItemEntity(item, invoice, queryRunner.manager),
      );
      invoice.items = items;
      invoice.totalCents = items.reduce(
  ...
  async findAll(query: InvoiceQueryDto) {
    ...
    if (query.supplier) {
      qb.andWhere("invoice.supplierId = :supplier", {
  ...
  private async recalculateTotals(invoiceId: string, manager: EntityManager) {
    const invoice = await manager.findOneOrFail(Invoice, {
      where: { id: invoiceId },
      relations: ["items"],
    });
