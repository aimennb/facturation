...
export class InvoicesController {
  ...
  @Patch(':id')
  @Roles('ADMIN', 'VENDEUR')
  update(@Param('id') id: string, @Req() req: Request, @Body() payload: UpdateInvoiceDto) {
    const user = req.user as { sub?: string } | undefined;
  ...
  @Post(':id/items')
  @Roles('ADMIN', 'VENDEUR')
  addItem(@Param('id') id: string, @Req() req: Request, @Body() payload: InvoiceItemDto) {
    const user = req.user as { sub?: string } | undefined;
  ...
  @Patch('items/:itemId')
  @Roles('ADMIN', 'VENDEUR')
  updateItem(@Param('itemId') itemId: string, @Req() req: Request, @Body() payload: InvoiceItemDto) {
    const user = req.user as { sub?: string } | undefined;
  ...
  @Get(':id/pdf')
  @Roles('ADMIN', 'VENDEUR', 'FOURNISSEUR')
  async pdf(@Param('id') id: string, @Query('locale') locale: 'fr' | 'ar', @Res() res: Response) {
